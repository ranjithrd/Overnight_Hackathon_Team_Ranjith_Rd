...
import { getUsers, addLoan } from '../../services/api';
import { Search, DollarSign, Calendar, Percent, FileText, CheckCircle, ArrowLeft, User } from 'lucide-react';

export default function ManagerLoanCreationPage({ onBack }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!duration || parseInt(duration) < 1 || parseInt(duration) > 360) {
      setError('Duration must be between 1 and 360 months');
      return;
    }
    ...
    try {
      await addLoan({
        borrower_id: selectedUser.user_id,
    ...
    return (
      <div style={{
        ...
        <p style={{ margin: '0 0 24px', color: '#e5e7eb', fontSize: '16px' }}>
          ₹{parseFloat(amount).toLocaleString('en-IN')} loan for {selectedUser?.name}
        </p>
      </div>
    );
  }
